...
describe('createFrameGesture', () => {
  ...
  it('activates on two-hand tips and returns a non-axis-aligned quad', () => {
    ...
    // Slightly skewed: not a perfect axis-aligned rectangle
    ...
    expect(r.quad!.points[0]).toEqual({ x: 280, y: 280 }) // left index
    expect(r.quad!.points[1]).toEqual({ x: 720, y: 320 }) // right index
    expect(r.quad!.points[2]).toEqual({ x: 680, y: 580 }) // right thumb
    expect(r.quad!.points[3]).toEqual({ x: 320, y: 550 }) // left thumb
  })
  ...
  it('activates using thumb/index tips even when other fingers are open', () => {
    const g = createFrameGesture({ lerpAlpha: 1 })
    const left = hand('Left', { index: [0.3, 0.3], thumb: [0.3, 0.55] })
    const right = hand('Right', { index: [0.7, 0.3], thumb: [0.7, 0.55] })
    for (const h of [left, right]) {
      for (const tip of [12, 16, 20]) {
        h.landmarks[tip] = { x: h.landmarks[8].x, y: h.landmarks[8].y }
        h.landmarks[tip - 2] = {
          x: h.landmarks[8].x,
          y: h.landmarks[8].y + 0.08,
        }
      }
    }
    const r = g.update([left, right], { width: 1000, height: 1000 }, 0)
  ...
  it('rejects frames with aspect ratio outside 1:4…4:1', () => {
    const g = createFrameGesture({ lerpAlpha: 1 })
    const size = { width: 1000, height: 1000 }
    const tooWideLeft = hand('Left', {
      index: [0.05, 0.35],
      thumb: [0.05, 0.45],
    })
    const tooWideRight = hand('Right', {
      index: [0.95, 0.35],
      thumb: [0.95, 0.45],
    })
    expect(g.update([tooWideLeft, tooWideRight], size, 0).phase).toBe('idle')

    const tooTallLeft = hand('Left', {
      index: [0.45, 0.05],
      thumb: [0.55, 0.05],
    })
    const tooTallRight = hand('Right', {
      index: [0.45, 0.95],
      thumb: [0.55, 0.95],
    })
